...
~ Version "1.5" >> Fixes
~ Version "1.6" >> Removed useless translations which were wrong and added English language for allied lads.
...
public void OnPluginStart()
{
	...
	g_CvarFreeVIP = CreateConVar("vip_od_danej_godziny", "1", "Should a FreeVIP for specific hours be enabled?");
	g_CvarVipFromHr = CreateConVar("vip_from_hr", "22", "From which hour should freevip start?");
	g_CvarVipToHr = CreateConVar("vip_to_hr", "8", "To which hour should be freevip assigned? (AM)");
	
	g_cvVipUnlimitedAmmo = CreateConVar("vip_unli_ammo", "0", "Should VIP get unlimited ammo?");
	
	AutoExecConfig(true, "H2K_VipConf_2.0");
}
...
public Action Event_PlayerSpawn(Event hEvent, const char[] chName, bool bDontBroadcast)
{
	...
	if (IsPlayerVip(client))
	{
		...
		if (RoundCount >= g_CvarGrenadesRound.IntValue)
			CreateTimer(0.1, TimerPlayerSpawn, client, TIMER_FLAG_NO_MAPCHANGE);
	}
...
public Action TimerPlayerSpawn(Handle timer, int client)
{
	...
	if (g_CvarHealGranat.BoolValue)
		if (GetEntProp(client, Prop_Send, "m_iAmmo", _, g_iOShealth) < 1)
	...
	if (g_CvarTaGranat.BoolValue)
		if (GetEntProp(client, Prop_Send, "m_iAmmo", _, g_iOSta) < 1)
			GivePlayerItem(client, "weapon_tagrenade");
	
	if (GetClientTeam(client) == CS_TEAM_CT && g_CvarIncGranat.BoolValue && GetEntProp(client, Prop_Send, "m_iAmmo", _, g_iOSinc) < 1)
			GivePlayerItem(client, "weapon_incgrenade");
	
	if (GetClientTeam(client) == CS_TEAM_T && g_CvarMolotovGranat.BoolValue && GetEntProp(client, Prop_Send, "m_iAmmo", _, g_iOSmol) < 1)
...
public void GrenadeOffsets()
{
	...
	ent = CreateEntityByName("weapon_healthshot");
	DispatchSpawn(ent);
	g_iOShealth = GetEntProp(ent, Prop_Send, "m_iPrimaryAmmoType");
	AcceptEntityInput(ent, "Kill");
	...
	ent = CreateEntityByName("weapon_tagrenade");
	DispatchSpawn(ent);
	g_iOSta = GetEntProp(ent, Prop_Send, "m_iPrimaryAmmoType");
	AcceptEntityInput(ent, "Kill");
}
